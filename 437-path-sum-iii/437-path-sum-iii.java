...
class Solution {
    ...
    void bruteForce(TreeNode root,int target){
        ...
        everyNodeAsRoot(root,target,0);

        bruteForce(root.left,target);
        bruteForce(root.right,target);
        
    }
    void everyNodeAsRoot(TreeNode root,int target,int sum){
        if(root==null)return;
        
        if(sum+root.val==target){
            count++;
        }
        everyNodeAsRoot(root.left,target,sum+root.val);
        everyNodeAsRoot(root.right,target,sum+root.val);
