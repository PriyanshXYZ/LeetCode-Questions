    ...
    long verticallyDownBST(Node root,int target)
    {
        if(dfs(root,target)){
            ...
            while(q.size()>0){
                ...
                while(size-- > 0){
                    Pair rem = q.remove();
                    if(rem.path==0)sum+=rem.node.data;
                    if(rem.node.left!=null)q.add(new Pair(rem.node.left,rem.path-1));
                    if(rem.node.right!=null)q.add(new Pair(rem.node.right,rem.path+1));
                }
            }
            return sum-tar.data;
        }
